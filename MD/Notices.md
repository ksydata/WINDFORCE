...
※ 1차 평가는 평균 예측오차율 NMAE와 정산금획득률을 기반으로 진행되고, 상위 팀은 2차 발표평가로 이어집니다.

즉 워크샵에서 들어야 할 질문은 "어떤 모델이 좋나요?" 하나가 아닙니다. 데이터가 어떤 시간 단위로 주어지는지, 예측 오차와 정산금 관점이 어떻게 함께 반영되는지, 발표평가에서 과제 이해도와 적용 가능성을 어떻게 보여줄지가 더 중요합니다.
...
발표평가까지 고려하면 모델 성능 외에 어떤 설명 자료가 필요한가?

팀원이 있다면 데이터 전처리, 검증, 제출, 발표 역할을 어떻게 나눌 것인가?
